...
func (pq *ProjectQuery) QueryRoleBindings() *RoleBindingQuery {
	query := &RoleBindingQuery{config: pq.config}
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withRoleBindings: pq.withRoleBindings.Clone(),
...
func (pq *ProjectQuery) WithRoleBindings(opts ...func(*RoleBindingQuery)) *ProjectQuery {
	query := &RoleBindingQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withRoleBindings = query
	return pq
}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context) ([]*Project, error) {
	...
	if query := pq.withRoleBindings; query != nil {
		...
		query.Where(predicate.RoleBinding(func(s *sql.Selector) {
			s.Where(sql.InValues(project.RoleBindingsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.project_role_bindings
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "project_role_bindings" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "project_role_bindings" returned %v for node %v`, *fk, n.ID)
			}
