...
func (sq *ServiceQuery) QueryRoleBindings() *RoleBindingQuery {
	query := &RoleBindingQuery{config: sq.config}
...
func (sq *ServiceQuery) QueryServers() *ServerQuery {
	query := &ServerQuery{config: sq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *ServiceQuery) Clone() *ServiceQuery {
	...
	return &ServiceQuery{
		...
		withRoleBindings: sq.withRoleBindings.Clone(),
...
func (sq *ServiceQuery) WithRoleBindings(opts ...func(*RoleBindingQuery)) *ServiceQuery {
	query := &RoleBindingQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withRoleBindings = query
	return sq
}
...
func (sq *ServiceQuery) WithServers(opts ...func(*ServerQuery)) *ServiceQuery {
	query := &ServerQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *ServiceQuery) sqlAll(ctx context.Context) ([]*Service, error) {
	...
	if query := sq.withRoleBindings; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Service)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.RoleBindings = []*RoleBinding{}
		}
		query.withFKs = true
		query.Where(predicate.RoleBinding(func(s *sql.Selector) {
			s.Where(sql.InValues(service.RoleBindingsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.service_role_bindings
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "service_role_bindings" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "service_role_bindings" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.RoleBindings = append(node.Edges.RoleBindings, n)
		}
	}
	...
	if query := sq.withServers; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(service.ServersPrimaryKey[0], fks...))
			},
