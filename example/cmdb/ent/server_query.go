...
func (sq *ServerQuery) QueryServices() *ServiceQuery {
...
func (sq *ServerQuery) Clone() *ServerQuery {
	...
	return &ServerQuery{
		...
		withServices: sq.withServices.Clone(),
...
func (sq *ServerQuery) WithServices(opts ...func(*ServiceQuery)) *ServerQuery {
	query := &ServiceQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withServices = query
	return sq
}
...
func (sq *ServerQuery) sqlAll(ctx context.Context) ([]*Server, error) {
	...
	if query := sq.withServices; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(server.ServicesPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, sq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "services": %w`, err)
		}
