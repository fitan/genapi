...
func (stq *ServiceTreeQuery) Unique(unique bool) *ServiceTreeQuery {
	stq.unique = &unique
	return stq
}
...
//		Scan(ctx, &v)
//
func (stq *ServiceTreeQuery) Select(fields ...string) *ServiceTreeSelect {
...
func (stq *ServiceTreeQuery) sqlAll(ctx context.Context) ([]*ServiceTree, error) {
	...
	if query := stq.withProject; query != nil {
		...
		for i := range nodes {
			if nodes[i].service_tree_service == nil {
				continue
			}
