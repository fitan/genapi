...
func (mq *MessageQuery) sqlAll(ctx context.Context) ([]*Message, error) {
	...
	if query := mq.withUser; query != nil {
		...
		query.Where(predicate.User(func(s *sql.Selector) {
			s.Where(sql.InValues(message.UserColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_msg
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_msg" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_msg" returned %v for node %v`, *fk, n.ID)
			}
