...
func (uq *UserQuery) QueryRoleBindings() *RoleBindingQuery {
...
func (uq *UserQuery) WithRoleBindings(opts ...func(*RoleBindingQuery)) *UserQuery {
	query := &RoleBindingQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withRoleBindings = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withRoleBindings; query != nil {
		...
		query.Where(predicate.RoleBinding(func(s *sql.Selector) {
			s.Where(sql.InValues(user.RoleBindingsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_role_bindings" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_role_bindings" returned %v for node %v`, *fk, n.ID)
			}
