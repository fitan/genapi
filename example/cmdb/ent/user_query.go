...
// QueryRoleBind chains the current query on the "role_bind" edge.
func (uq *UserQuery) QueryRoleBind() *RoleBindingQuery {
	query := &RoleBindingQuery{config: uq.config}
...
// WithRoleBind tells the query-builder to eager-load the nodes that are connected to
// the "role_bind" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithRoleBind(opts ...func(*RoleBindingQuery)) *UserQuery {
	query := &RoleBindingQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withRoleBind = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withRoleBind; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.RoleBind = []*RoleBinding{}
		}
		query.withFKs = true
		query.Where(predicate.RoleBinding(func(s *sql.Selector) {
			s.Where(sql.InValues(user.RoleBindColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_role_bind
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_role_bind" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_role_bind" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.RoleBind = append(node.Edges.RoleBind, n)
		}
	}
	...
	if query := uq.withAlert; query != nil {
		...
		nodeids := make(map[int][]*User)
		for i := range nodes {
			if fk := nodes[i].user_alert; fk != nil {
				ids = append(ids, *fk)
