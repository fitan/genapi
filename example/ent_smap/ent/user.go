...
type User struct {
	...
	// Age1 holds the value of the "age1" field.
	Age1 int `json:"age1,omitempty"`
	// En holds the value of the "en" field.
	En user.En `json:"en,omitempty"`
...
func (*User) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		switch columns[i] {
		case user.FieldID, user.FieldAge1:
			values[i] = &sql.NullInt64{}
		case user.FieldName, user.FieldEn:
	...
	for i := range columns {
		...
		case user.FieldAge1:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field age1", values[i])
			} else if value.Valid {
				u.Age1 = int(value.Int64)
			}
		case user.FieldEn:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field en", values[i])
			} else if value.Valid {
				u.En = user.En(value.String)
			}
		}
	}
	return nil
...
func (u *User) String() string {
	...
	builder.WriteString(", age1=")
	builder.WriteString(fmt.Sprintf("%v", u.Age1))
	builder.WriteString(", en=")
	builder.WriteString(fmt.Sprintf("%v", u.En))
	builder.WriteByte(')')
